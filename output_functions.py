...
def build_on_tag_review(death_on_tag, players, tid_date_time):
	...
	for name_prof in death_on_tag:
		...
		row = f"|<span class='tooltip tooltip-right' data-tooltip=' {account}'> {player} </span> | {{{{{profession}}}}} {profession[:3]} | {active_time:,.1f} | {avg_dist} | {on_tag} | {off_tag} | {after_tag} | {run_back} | {total} |{off_tag_ranges} |"
		rows.append(row)
	...
	for player, player_data in top_stats['player'].items():
		...
		player_apm_data = {
			...
			"ActionsPerMinute": round(top_stats["skill_casts_by_role"][profession][name_prof_acct].get(apm_mode, 0)/(player_active_time/60),2) 
									if name_prof_acct in top_stats["skill_casts_by_role"][profession] else 0,
			"BoonScore": boon_ps,
